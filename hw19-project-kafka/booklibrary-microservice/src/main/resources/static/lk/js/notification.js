...
stompClient.onConnect = (frame) => {
    ...
    stompClient.subscribe(topic, (e) => {
        ...
        if (json.before && document.getElementById(json.before.id)) {
            entities.removeChild(document.getElementById(json.before.id));
        }
        if (json.after) {
            entities.innerHTML += buildEntityRow(json.after);
        }
        if (granted) {
